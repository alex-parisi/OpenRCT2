...
// Several lookup functions index a `static map[]` directly without bounds-checking
// (`return map[x]`); callers are trusted to pass values within range. We mirror
// those sizes here so the sweeps below cannot trigger out-of-bounds reads under
// ASan / UBSan and remain valid as the underlying tables grow.
namespace
{
